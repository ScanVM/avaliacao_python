...
while loop == "sim":
    ...
    if choice == 1:
        os.system('cls')
        if len(id) > 0:
            choice2 = int(input("Escolha a forma de consulta:\n1 - ID\n2 - Locador\n3 - Todos os elementos\n"))
            #CONSULTA PELO ID
            if choice2 == 1:
                os.system('cls')
                ...
                if numID in id:
                    for search in id:
                        if search == numID:
                            ...
                            print("ID: {}\nLocador: {}\nValor: R$ {}\nTamanho: {} m²\nCômodo(s): {}\nDescrição: {}\n".format(id[position],name[position],value[position],size[position],homeRoom[position],description[position]))
                    time.sleep(3)
                    print('\n\n\n')
                    loop = "sim"
                else:
            ...
            #CONSULTA PELO NOME DE LOCADORES
                ...
                os.system('cls')
                nameLocator = input("Informe o nome do locador do imóvel: \n").lower()
                #VERIFICAÇÃO DA EXISTENCIA DO LOCADOR
                if nameLocator in name:
                    for search in range(len(name)):
                        if name[search] == nameLocator:
                            print("\n")
                            print("ID: {}\nLocador: {}\nValor: R$ {}\nTamanho: {} m²\nCômodo(s): {}\nDescrição: {}\n".format(id[search],name[search],value[search],size[search],homeRoom[search],description[search]))
                            time.sleep(2)
                    print("\n\n\n")
                    time.sleep(3)
                    loop = "sim"
                                ...
                                for indexHomeRoom in range(len(homeRoom)):
                                    for indexDescription in range(len(description)):
                                        if indexId == indexName == indexValue == indexSize == indexHomeRoom == indexDescription:
                                            print("\n")
                                            print("ID: {}\nLocador: {}\nValor: R$ {}\nTamanho: {} m²\nCômodo(s): {}\nDescrição: {}\n".format(id[indexId],name[indexName],value[indexValue],size[indexSize],homeRoom[indexHomeRoom],description[indexDescription]))
                                            print("\n")
                                            time.sleep(2)     
        else:
            ...
            time.sleep(2)
            os.system('cls')
            print("Para acessar essa função é necessário um cadastro, redirecionando ao menu...\n")
            time.sleep(4)
            os.system('cls')
            loop = "sim"
    #FUNÇÃO DE CADASTRO DOS IMÓVEIS
        ...
        if primaryKey in id:
            ...
            HomeRoom = int(input("Informe quantos cômodos existem no imóvel: \n"))
    ...
    #FUNÇÃO DE EDIÇÃO DOS ELEMENTOS
        ...
        os.system('cls')
        if len(id) > 0:
            choiceEdit = int(input("Informe o elemento que será alterado:\n1 - ID\n2 - Locador\n3 - Valor\n4 - Tamanho\n5 - Cômodo(s)\n6 - Descrição\n"))
            if choiceEdit < 0 or choiceEdit > 6:
                ...
                time.sleep(2)
                os.system('cls')
                edition = int(input("Digite o ID do cadastro a ser editado: \n"))
                #VERIFICAÇÃO DA EXISTENCIA DO ID 
                if edition in id:
                #EDIÇÃO DO ID
                    if choiceEdit == 1:
                    ...
                    #EDIÇÃO DE LOCADOR
                        ...
                        os.system('cls')
                        newLocator = input("Informe o nome do novo locador: \n")
                        for searchEdit in id:
                            if searchEdit == edition:
                                positionEdit = id.index(edition)
                                name[positionEdit] = newLocator
                        os.system('cls')
                        print("Locador alterado com sucesso !\n\nNão esqueça de gravar os dados no menu ! :D")
                        time.sleep(3)
                        os.system('cls')
                        loop = "sim"
                    #EDIÇÃO DO VALOR
                        ...
                        os.system('cls')
                        newValue = float(input("Informe o novo valor do imóvel: \n"))
                        for searchEdit in id:
                            if searchEdit == edition:
                                positionEdit = id.index(edition)
                    ...
                    #EDIÇÃO DO TAMANHO
                        ...
                        os.system('cls')
                        newSize = float(input("Informe o novo tamanho em m²: \n"))
                        for searchEdit in id:
                            if searchEdit == edition:
                                positionEdit = id.index(edition)
                    ...
                    #EDIÇÃO DA QUANTIDADE DE COMODOS
                        ...
                        os.system('cls')
                        newHomeRoom = input("Informe o novo número de cômodos: \n")
                        for searchEdit in id:
                            if searchEdit == edition:
                                positionEdit = id.index(edition)
                                homeRoom[positionEdit] = newHomeRoom
                        os.system('cls')
                        print("Quantidade de cômodos alterada com sucesso !\n\nNão esqueça de gravar os dados no menu ! :D")
                        time.sleep(3)
                        os.system('cls')
                        loop = "sim"
                    #EDIÇÃO DA DESCRIÇÃO
    ...
    #FUNÇÃO DE EXCLUIR OS ELEMENTOS
        ...
        os.system('cls')
        if len(id) > 0:
            idDelete = int(input("Informe o ID do cadastro que deseja excluir:\n"))
            #VERIFICAÇÃO DA EXISTENCIA DO ID
            if idDelete in id:
                for searchDelete in id:
                        if searchDelete == idDelete:
                            positionDelete = id.index(idDelete)
                confirmationDelete = int(input("O elemento será excluido permanentemente, gostaria de prosseguir ?\n1 - Sim\n2 - Não\n"))
    ...
    #FUNÇÃO DE SAIDA 
        ...
        os.system('cls')
        questionSave = (int(input("O programa está fechando, não esqueça de gravar os seus dados no menu >.<\nSelecione:\n1 - Sair\n2 - Voltar ao menu")))
        if questionSave == 1:
            os.system('cls')
            print("Saindo...")
            time.sleep(3)
